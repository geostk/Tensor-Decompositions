...
function [Core_Tensor_A,Singular_Factors_A,Core_Tensor_B,Singular_Factors_B]=Coupled_HOSVD(Tensor_A,Tensor_B,Coupled_Modes_AB,Rank)
...
Decomposed_A=tucker_als(Tensor_A,Rank);

for i=1:iter_A
    Singular_Factors_A{i,1}=Decomposed_A.U{i};
end

iter_B=ndims(Tensor_B);
Singular_Factors_B=cell(iter_B,1);
Decomposed_B=tucker_als(Tensor_B,Rank);

for i=1:iter_B
    Singular_Factors_B{i,1}=Decomposed_B.U{i};
end
...
for i=1:Number_coupled_modes
    ...
    Coupled_decomposed=tucker_als((tensor([Matricize_A.data Matricize_B.data])),Rank);
    
    Singular_Factors_A{Coupled_mode_A,1}=Coupled_decomposed.U{1};
    Singular_Factors_B{Coupled_mode_B,1}=Coupled_decomposed.U{1};
